...
import part3.model.ImportData;
import part3.model.ImportDataDetail;
import part3.utils.DB;
...
public class ImportDataService {
	...
	public List<ImportDataDetail> getImportDataDetailsByMainId(String importDataId) {
		...
		try {
			...
			while (rs.next()) {
				...
				i.setImportId(rs.getString("importid"));
				i.setCgbz("1".equals(rs.getString("cgbz")) ? "�Ѵ���" : "δ����");
	...
	public void saveStudents(String importId) {
		...
		String sql = "insert into t_student(stunum,stuname,stuage,stusex,stubirthday,stuhobby) "
				+ "select col0,col1,col2,col3,col4,col5 from t_importdatadetail " + " where importid = '" + importId
				+ "'";
		PreparedStatement ps = DB.prepare(conn, sql);
		try {
			ps.executeUpdate();
	...
	public void updImportDataDetailStatus(String importId) {
		Connection conn = DB.createConn();
		String id = Long.toString(System.currentTimeMillis());
		String sql = "update t_importdatadetail set cgbz ='1',hcode='��',msg='����ɹ���' where importid = ?";
